...
def main():
    ...
    try:
        ...
        if 'teff' in table.colnames:
            table = table[(table['teff'] > 3000) & (table['teff'] < 15000)]
        
        if 'logg' in table.colnames:
            table = table[(table['logg'] > 0) & (table['logg'] < 6)]
        
        if 'feh' in table.colnames:
            table = table[(table['feh'] > -5) & (table['feh'] < 1)]
        
        if 'snr' in table.colnames:
            table = table[table['snr'] > 5]

        print(f"过滤后: {len(table)} / {initial_count} 条")
